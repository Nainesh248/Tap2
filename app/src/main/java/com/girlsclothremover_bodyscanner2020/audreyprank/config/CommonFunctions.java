...
import com.girlsclothremover_bodyscanner2020.audreyprank.model.LoginResponse;
import com.girlsclothremover_bodyscanner2020.audreyprank.reciver.ConnectivityReceiver;
...
import static com.girlsclothremover_bodyscanner2020.audreyprank.config.Constants.BITMAP_SCALE;
...
public class CommonFunctions {
    ...
    public static String convertdateEvents(String date) {
        String result = "";
        try {
            String myFormat = "dd-MM-yyyy hh:mm a"; //In which you need put here dd-MM-yyyy
            String myFormat1 = "yyyy-MM-dd HH:mm:ss"; //In which you need put here dd-MM-yyyy

            SimpleDateFormat sdf = new SimpleDateFormat(myFormat, Locale.US);
            SimpleDateFormat sdf1 = new SimpleDateFormat(myFormat1, Locale.US);
            Date startdatedate = sdf1.parse(date);

            result = sdf.format(startdatedate.getTime());
            return result;
    ...
    public static void sendsmswhatsap(Activity activity, String number) {
        try {
            ...
            /*String smsNumber = number.toString().trim().replace(" ","");
            smsNumber=smsNumber.replace("+","");// E164 format without '+' sign;
            Log.e("sendsmswhatsap",smsNumber);
            Intent sendIntent = new Intent(Intent.ACTION_SEND);
            sendIntent.setType("text/plain");
            sendIntent.putExtra(Intent.EXTRA_TEXT, activity.getString(R.string.hi));
   ...
   /* public static UserDetailResponse getuserdetailresponse(Context context) {
    ...
    public static void getCurrentLocation(Activity activity) {
        ...
        if (checkPermission(activity, Manifest.permission.ACCESS_FINE_LOCATION)) {
            ...
            @SuppressLint("MissingPermission") Location lastKnownLocationNetwork = locationManager.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);
  ...
  /*  public static void changeActivity(Activity activity, Class cls, Bundle bundle, boolean isfinish, boolean isSlideOutRight, boolean isClearTop) {
    ...
    public static Bitmap fastblur(Bitmap sentBitmap) {
        ...
        int radius = Constants.BLUR_RADIUS;
